function [x,yAll,MSE,traceP,z] = OptimizeSqEnKF(n,T,var_y,skip,Gap,NeAll,LocRadAll,inflAll,Save,EXIT)
...
for zz=1:length(NeAll)
    ...
    for pp = 1:length(LocRadAll)
        ...
        for tt = 1:length(inflAll)
            ...
            [x,traceP] = mySqEnKF(infl,Ne,Xo,Cloc,z,R,H,F,Gap,Steps,dt);
            MSE = sqrt(mean( (x(:,Gap+1:Gap:end)-yAll(:,Gap+1:Gap:end)).^2 ));
            traceP = sqrt(traceP'/n);
            if Save == 1
                Filename = strcat('SqEnKF_Gap_',num2str(Gap),'_Ne_',num2str(Ne), ...
