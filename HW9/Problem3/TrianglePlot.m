...
axisXYtmp = [leftEdge riteEdge botEdge topEdge]+[-padX padX -padY padY]; % do we need this much padding really?
axisXY = [min(leftEdge,botEdge) max(riteEdge,topEdge) min(leftEdge,botEdge) max(riteEdge,topEdge)] ...
            +[-max(padX,padY) max(padX,padY) -max(padX,padY) max(padX,padY)];
rangeX = riteEdge-leftEdge;
rangeY =  topEdge -botEdge;
