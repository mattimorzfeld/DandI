function [x,yAll,MSE,traceP,z] = OptimizeEnsembleOfVar(n,T,var_y,skip,Gap,NeAll,ClocRadAll,inflAll,Save,EXIT)
...
for zz=1:length(NeAll)
    ...
    for pp = 1:length(ClocRadAll)
        ...
        for tt = 1:length(inflAll)
            ...
            [x,traceP,~]=myEnsembleOfVar(infl,Cloc,Ne,Xo,muo,Lb,z,R,H,F,Gap,Steps,dt);
            MSE = sqrt(mean( (x(:,Gap+1:Gap:end)-yAll(:,Gap+1:Gap:end)).^2 ));
            traceP = sqrt(traceP);
            if Save == 1
                Filename = strcat('EnsembleVar_Gap_',num2str(Gap),'_Ne_',num2str(Ne), ...
